...
--1
select tariff,
       uniqExact(views.idhash_view) as total_view,
       countIf(views.idhash_order, views.idhash_order > 0) as total_order,
       countIf(orders.da_dttm, isNotNull(orders.da_dttm)) as have_driver,
       countIf(orders.rfc_dttm, isNotNull(orders.rfc_dttm)) as have_cer,
       countIf(orders.cc_dttm, isNotNull(orders.cc_dttm)) as sit,
       countIf(orders.finish_dttm, isNotNull(orders.finish_dttm)) as trip_was
from views
    left join orders on orders.idhash_order = views.idhash_order --mb outer
group by tariff;

--2
select idhash_client as client,
       topK(4)(tariff) as top_tarrif,
       uniqExact(tariff) as count_tarrif
from views
group by idhash_client;

--3
select geoToH3(views.latitude, views.longitude, 7) as geo_H3,
       uniqExact(views.idhash_order) as orders
from views
    inner join orders on views.idhash_order = orders.idhash_order
where toHour(views.view_dttm) >= 7 and toHour(views.view_dttm) < 10 and isNotNull(orders.finish_dttm)
group by geo_H3
order by orders desc
limit 10;

--c 18 до 20 аналогично, изменим условие where.

--4
select median(dateDiff('second', order_dttm, da_dttm)) as median,
       quantile(0.95)(dateDiff('second', order_dttm, da_dttm)) as quantile
from orders;
